...
The CO2 data has a positive trend and increases steadily and pretty linearly over time.
...
co2 |> 
  group_by(month) |> 
  summarise(monthMean = mean(monthly_average)) |> 
  ggplot(aes(x = month, y = monthMean)) +
  geom_line() +
...
Within a year, CO2 increases in the first half of the year, decreases for a few months, then starts to increase again. Over all of the years of data, CO2 peaks in May and is lowest in September and October. This is likely explained by plants increasing their photosynthesis rates in the warmer months in the northern hemisphere(May - September) and photosynthesis uses CO2. When photosynthesis is slowed down, CO2 increases in the colder months.
...
The data comes from NASA's Goddard Institute for Space Studies and represents the Land-Ocean Temperature Index, which is a measure of global temperature anomalies. This data is comes from meteorological station records, sea surface temperature observations, and Antarctic research stations.

The first column is the year, which is obviously an integer and goes from 1880 to present. The next column is No_Smoothing which is a numerical value of the annual temperature anomaly measured in degrees Celsius. The last column is the Lowess(5) column which is a numerical value of the smoothed temperature anomaly using a LOWESS regression with a 5% smoothing factor measured in degrees Celsius. I did have to consult an AI LLM (chatGPT) to help me better understand the LOWESS column of the dataset.

Global temperature refers to the average surface temperature of Earth in a given year. Global temperature anomaly measures the deviation from a long-term average baseline temperature (1951–1980).
...
```{r}
globalMeans <-  read_table("https://data.giss.nasa.gov/gistemp/graphs/graph_data/Global_Mean_Estimates_based_on_Land_and_Ocean_Data/graph.txt", 
                  comment="#",
                  col_names = c("Year", "AnnualMean", "LowessSmoothing"),
                  skip = 5)
```
...
I see an overall positive trend in the plot. This means that over time, the global temperature is deviating more from the long-term average from 1951 to 1980. It also means that the average global temperature is increasing over time.
...
I used the prompt: "how to calculate a rolling average in R with example" in chatGPT. It gave me a few outputs with different packages, and I decided on the slider package because its within tidyverse. I had to switch the column names and add more columns from the example output to get what I needed, but the code provided worked well.
...
The annual average represents the temperature anomaly for a single year. The 5-year average smooths out these fluctuations by taking the mean of five consecutive years, providing a clearer picture of long-term trends.

```{r}
library(slider)
globalMeansRunning <- globalMeans |> 
  mutate(
    rolling5yr  = slide_dbl(AnnualMean, mean, .before = 2, .after = 2, .complete = TRUE),
    rolling10yr = slide_dbl(AnnualMean, mean, .before = 4, .after = 5, .complete = TRUE),
    rolling20yr = slide_dbl(AnnualMean, mean, .before = 9, .after = 10, .complete = TRUE)
  ) |> 
  select(-LowessSmoothing)
globalMeansRunning
```
...
```{r}
globalRollingLong <- globalMeansRunning |> 
  pivot_longer(cols = c(rolling5yr, rolling10yr, rolling20yr), 
               names_to = "Type", values_to = "averageTemp")
globalRollingLong |> 
  ggplot(aes(x = Year, y = averageTemp, color = Type)) +
  geom_line() +
  theme_bw()  +
  scale_x_continuous(breaks = seq(1880, 2025, 10)) +
  ylab("Global temperature anomaly (C)")
...
The rolling 5 year average fluctuates the most, then the rolling 10 year, and then the rolling 20 year. This makes sense because a smaller window like 5 years reacts more to short-term fluctuations when a larger window like 10 or 20 years smooths those fluctuations, focusing on long-term patterns.
...
There is evidence of a pause in global warming when using rolling averages. There appears to be a brief flattening of the lines from 1950 to around 1975, which can be seen most in the rolling 10 and 20 year lines. There is still fluctuation in the 5 year line especially, but there does appear to be a small pause there.
...
The columns are depth in meters, age of the ice in years, age of the air in years, and CO2 concentration in ppmv. The data comes from ice core samples extracted at Vostok Station, and the CO₂ values represent past atmospheric conditions. The temporal resolution of the data is approximately 400,000 years, with measurements taken at irregular intervals (sometimes thousands of years apart).

```{r}
iceCoreData <- read_table("vostok.icecore.co2", 
                        col_names = c("Depth", "IceAge", "AirAge", "CO2"),
                  skip = 20)

iceCoreData <- iceCoreData |> 
  mutate(year = 2003 - IceAge) |> 
  arrange(year)
...
co2 <- co2 |> 
  group_by(year) |> 
  summarize(CO2 = mean(monthly_average)) |> 
  select(year, CO2)


combinedData <- bind_rows(iceCoreData, co2)
...
As seen in the plot, atmospheric CO2 levels fluctuate significantly between years. However, it can be seen that towards the right side of the plot the levels shoot up, which is when the data is getting closer to the present. I created another plot with the x axis scale logged, and you can see this steady increase in that plot.
